""" Apache license from tf
"""

# This document kept for future use, but did not end up getting used for final report.
...
def _LoadModel(gd_file, ckpt_file):
  ...
  with tf.Graph().as_default():
    ...
    sess = tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(allow_soft_placement=True))  # Place ops on CPU if no GPU is present
...
def get_softmax(sess, t, sent_id, chars_id):
  outputs = []
  sess.run(t['states_init'])
  for word, chars in zip(sent_id, chars_id):
    softmax = sess.run(t['softmax_out'], feed_dict={
      t['char_inputs_in']: np.expand_dims(chars, (0, 1)),  # Shape (1, 1, MAX_WORD_LEN) = (1, 1, 50)
...
def softmax_sent(sent, vocab_file='lm_1b/vocab-2016-09-10.txt', pbtxt='lm_1b/graph-2016-09-10.pbtxt', ckpt='lm_1b/ckpt-*'):
  ...
  chars_id = [vocab.word_to_chars_ids(w) for w in sent.split()]  # List of numpy arrays of shape (MAX_WORD_LEN,) = (50,)
  
  outputs = get_softmax(sess, t, sent_id, chars_id)
  preds = [vocab.id_to_word(np.argmax(o)) for o in outputs]
  print(preds)
